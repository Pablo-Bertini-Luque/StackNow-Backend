...
const isAutenticated = (req, res, next) => { //Middleware para las rutas protegidas, solo son accesibles si el token es correcto
    const access_token = req.headers.access_token;
    if(!access_token){return res.status(401).json({success: false, message: 'User is not authorized'})};
    ...
    if(!user){return res.status(401).json({success: false, message: 'User is not authorized'})};
    req.user = user;
    next();
}
