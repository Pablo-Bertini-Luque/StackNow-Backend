...
const updateUser = async(req, res) => {
    ...
    try {
        if(req.file){
            const update = await User.findOneAndUpdate({id: userID}, {avatar: `${host}:${port}/public/avatars/${req.file.filename}`}, {new: true} );
            console.log(req.body)
            return (res.status(201).json({message: 'Avatar Updated', update}));
        }
        const updateData = req.body;
        const update = await User.findOneAndUpdate({id: userID}, updateData, {new: true});
